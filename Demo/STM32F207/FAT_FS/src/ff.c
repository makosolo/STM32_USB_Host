...
/*-----------------------------------------------------------------------*/
/* Mount/Unmount a Logical Drive  【挂载/卸载一个逻辑驱动器】                                       */
/*-----------------------------------------------------------------------*/

FRESULT f_mount (
	BYTE vol,		/* Logical drive number to be mounted/unmounted 【要安装逻辑驱动器号/卸载】*/
	FATFS *fs		/* Pointer to new file system object (NULL for unmount)【指针指向新的文件系统对象】*/
)
{
	FATFS *rfs;
...
FRESULT f_open (
	FIL *fp,			/* Pointer to the blank file object */		//【空白文件对象结构指针】
...
FRESULT f_read (
	FIL *fp, 		/* Pointer to the file object 【文件对象结构的指针】*/
	void *buff,		/* Pointer to data buffer 【存储读取数据的缓冲区的指针】*/
	UINT btr,		/* Number of bytes to read 【要读取的字节数】*/
	UINT *br		/* Pointer to number of bytes read 【返回已读取字节数变量的指针】*/
)
{
	FRESULT res;
...
FRESULT f_write (
	FIL *fp,			/* Pointer to the file object */			//【文件对象结构的指针】
	const void *buff,	/* Pointer to the data to be written */		//【存储写入数据的缓冲区的指针】
	UINT btw,			/* Number of bytes to write */				//【要写入的字节数】
...
FRESULT f_close (
	FIL *fp		/* Pointer to the file object to be closed 【文件对象结构的指针】*/
)
{
	FRESULT res;
...
FRESULT f_opendir (
	DIR *dj,			/* Pointer to directory object to create */	//【空白目录对象结构的指针】
...
FRESULT f_readdir (
	DIR *dj,			/* Pointer to the open directory object */	//【指向打开的目录对象结构的指针】
	FILINFO *fno		/* Pointer to file information to return */	//【指向文件信息结构的指针】
)
{
	FRESULT res;
	DEF_NAMEBUF;
...
FRESULT f_getfree (
	const TCHAR *path,	/* Path name of the logical drive number */							//【驱动器的根目录】
	DWORD *nclst,		/* Pointer to a variable to return number of free clusters */		//【存储空闲簇数目变量的指针】
	FATFS **fatfs		/* Pointer to return pointer to corresponding file system object */	//【文件系统对象指针的指针】 
)
{
	FRESULT res;
	FATFS *fs;
...
/*-----------------------------------------------------------------------*/
/* Truncate File
【执行此函数要求文件以读写发送打开，以只读方式打开此函数失败。】*/
/*-----------------------------------------------------------------------*/

FRESULT f_truncate (
	FIL *fp		/* Pointer to the file object */	//【文件对象结构】
)
{
	FRESULT res;
	DWORD ncl;
